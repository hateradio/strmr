...
class CommandYtdl {
    static make(state) {
        ...
        const commands = episodeData.slice(0, episodes > 0 ? episodes : 0).filter(ep => ep).map(ep => {
            ...
            const cmd = `# ${id}\nyoutube-dl -f "bestvideo[height<=${quality.replace('p', '')}]+bestaudio/best[height<=${quality.replace('p', '')}]" ${subcmd} --external-downloader aria2c --external-downloader-args "-c -j ${threads} -x ${threads} -s ${threads} -k 10M" --cookies cookies.txt ${uri} -o "${Command.escape(title)}.${id}${Command.showTitle(ep.title)}.${quality}.${extras}.%(ext)s"`
