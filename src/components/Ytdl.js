...
class Ytdl {
    static make(state) {
        ...
        const commands = episodeData.slice(0, episodes > 0 ? episodes : 0).filter(ep => ep).map(ep => {
            ...
            const cmd = `# ${id}\nyoutube-dl -f "bestvideo[height<=${quality.replace('p', '')}]+bestaudio/best[height<=${quality.replace('p', '')}]" ${subcmd} ${uri} -o "${Formatter.escape(title)}.${id}${Formatter.showTitle(ep.title)}.${quality}.${extras}.%(ext)s" --cookies cookies.txt ${log ? '&> ytdl.log' : ''}`.trim()
